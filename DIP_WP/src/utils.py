...
import numpy as np
import math
from torch_geometric.utils import to_undirected, from_scipy_sparse_matrix,dense_to_sparse,is_undirected, negative_sampling
...
def k_hop_subgraph(node_idx, num_hops, edge_index, max_nodes_per_hop = None,num_nodes = None, gpu_num = '0'):
...
def plus_edge(data_observed, label, p_edge, args):
 ...
 edge_index = data_observed.edge_index, max_nodes_per_hop=args.max_nodes_per_hop ,num_nodes=data_observed.num_nodes, gpu_num=args.gpu_num)
...
def prepare_data(args):
    ...
    #('Train_link:', str(len(train_graphs)),' Val_link:',str(len(val_graphs)),' Test_link:',str(len(test_graphs)))
...
def split_edges_wp(data,args):
    ...
    row, col = adj.nonzero(as_tuple=False).t()
    ...
    #sample negative edges
    neg_adj_mask = torch.ones(data.num_nodes, data.num_nodes, dtype=torch.uint8)
    # 保留mask主对角线以上的元素为true，其他的为false
    neg_adj_mask = neg_adj_mask.triu(diagonal=1).to(torch.bool)
    # 去掉边
    neg_adj_mask[row, col] = 0
    ...
    neg_row, neg_col = neg_adj_mask.nonzero(as_tuple=False).t()
    perm = torch.randperm(neg_row.size(0))[:n_t]
    # perm = torch.randperm(neg_row.size(0))[:300000]
    neg_row, neg_col = neg_row[perm], neg_col[perm]
    data.test_neg = torch.stack([neg_row, neg_col], dim=0)
    ...
    adj = torch.zeros(data.num_nodes, data.num_nodes, dtype=torch.uint8)
    adj[row, col] = 1
    adj[col, row] = 1
    node_deg = torch.sum(adj, dim=1)

    node_negfre = torch.zeros(data.num_nodes, dtype=torch.uint8)

    n_tot = n_v + data.train_pos.size(1)
    perm = torch.randperm(neg_row.size(0))[:n_tot]
    neg_subrow, neg_subcol = neg_row[perm], neg_col[perm]
    row, col = neg_subrow[:n_v], neg_subcol[:n_v]
    data.val_neg = torch.stack([row, col], dim=0)
    row, col = neg_subrow[n_v:], neg_subcol[n_v:]
    data.train_neg = torch.stack([row, col], dim=0)
    for i in range(row.size(0)):
        node_negfre[row[i]] = node_negfre[row[i]] + 1
        node_negfre[col[i]] = node_negfre[col[i]] + 1

    return data, node_deg, neg_row, neg_col, node_negfre

def plus_edgewp(data_observed, label, p_edge, args):
    nodes, edge_index_m, mapping, _ = k_hop_subgraph(node_idx=p_edge, num_hops=args.num_hops, \
                                                     edge_index=data_observed.edge_index,
                                                     max_nodes_per_hop=args.max_nodes_per_hop,
                                                     num_nodes=data_observed.num_nodes, gpu_num=args.gpu_num)
    x_sub = data_observed.x[nodes, :]
    edge_index_p = edge_index_m
    edge_index_p = torch.cat((edge_index_p, mapping.view(-1, 1)), dim=1)
    edge_index_p = torch.cat((edge_index_p, mapping[[1, 0]].view(-1, 1)), dim=1)
...
def minus_edgewp(data_observed, label, p_edge, args):
    nodes, edge_index_p, mapping, _ = k_hop_subgraph(node_idx=p_edge, num_hops=args.num_hops, \
                                                     edge_index=data_observed.edge_index,
                                                     max_nodes_per_hop=args.max_nodes_per_hop,
                                                     num_nodes=data_observed.num_nodes, gpu_num=args.gpu_num)
...
def set_init_attribute_representationwp(data,args, node_deg, neg_row, neg_col):
    #Construct data_observed and compute its node attributes & representation
    edge_index = torch.cat((data.train_pos,data.train_pos[[1,0],:]),dim=1)
    if args.observe_val_and_injection == False:
        data_observed = Data(edge_index=edge_index)
    ...
    if args.init_representation == 'gae':
        from gae_wei import CalGAE
        data_observed.x, auc, ap = CalGAE(edge_index_observed, x, val_and_test, args, node_deg, neg_row, neg_col)
    if args.init_representation == 'vgae':
        from vgae_wei import CalVGAE
        data_observed.x, auc, ap = CalVGAE(edge_index_observed, x, val_and_test, args, node_deg, neg_row, neg_col)
    if args.init_representation == 'argva':
        from argva_wei import CalARGVA
        data_observed.x, auc, ap = CalARGVA(edge_index_observed, x, val_and_test, args, node_deg, neg_row, neg_col)
    if args.init_representation != None:
        feature_results=[auc,ap]
    else:
        data_observed.x = x
...
def prepare_datawp(args):
    # load data from .mat or download from Planetoid dataset.

    if args.data_name in ('cora', 'citeseer', 'pubmed'):
        data = load_Planetoid_data(args)
        data, node_deg, neg_row, neg_col, node_negfre = split_edges_wp(data, args)
    elif args.data_name in ('chameleon', 'squirrel', 'film', 'cornell', 'texas', 'wisconsin'):
        datastr = args.data_name
        split_index = str(0)  ## this split is node-classification split from geom-gcn, not for link prediction
        splitstr = 'data/new_data_splits/' + datastr + '_split_0.6_0.2_' + split_index + '.npz'
    ...
    set_random_seed(args.seed)
    data_observed, feature_results = set_init_attribute_representationwp(data, args, node_deg, neg_row, neg_col)
    ...
    test_graphs = []
    for i in range(data.train_pos.size(1)):
        train_graphs.append(minus_edgewp(data_observed, 1, data.train_pos[:, i], args))

    for i in range(data.train_neg.size(1)):
        train_graphs.append(plus_edgewp(data_observed, 0, data.train_neg[:, i], args))

    for i in range(data.test_pos.size(1)):
        test_graphs.append(plus_edgewp(data_observed, 1, data.test_pos[:, i], args))

    for i in range(data.test_neg.size(1)):
        test_graphs.append(plus_edgewp(data_observed, 0, data.test_neg[:, i], args))
    if args.observe_val_and_injection == False:
        for i in range(data.val_pos.size(1)):
            val_graphs.append(plus_edgewp(data_observed, 1, data.val_pos[:, i], args))

        for i in range(data.val_neg.size(1)):
            val_graphs.append(plus_edgewp(data_observed, 0, data.val_neg[:, i], args))
    else:
        for i in range(data.val_pos.size(1)):
            val_graphs.append(minus_edgewp(data_observed, 1, data.val_pos[:, i], args))
    ...
    return train_loader, val_loader, test_loader, feature_results, node_deg, data.train_pos.size(1), node_negfre