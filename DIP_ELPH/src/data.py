...
from src.datasets.elph import get_hashed_train_val_test_datasets, make_train_eval_data, get_hashed_train_val_test_datasets_gae
...
def get_loaders(args, dataset, splits, directed, root):
...
def new_load_data(dataset_name, splits_file_path=None):
    graph_adjacency_list_file_path = os.path.join(par_dir, 'dataset', 'new_data', dataset_name, 'out1_graph_edges.txt')
    graph_node_features_and_labels_file_path = os.path.join(par_dir, 'dataset', 'new_data', dataset_name,
...
def split_edges_gc(data,args):
    split_edge = {}
    train_edge = {}
    valid_edge = {}
    test_edge = {}
    num_val = int(data.edge_index.shape[1]*args.val_pct)
    num_test = int(data.edge_index.shape[1] * args.test_pct)
    neg_edge = negative_sampling(data.edge_index, num_nodes=data.num_nodes,num_neg_samples=int((num_val+num_test)*1.2))
    perm = torch.randperm(data.edge_index.shape[1])
    valid_edge['edge'] = data.edge_index.t()[perm[:num_val], :]
    test_edge['edge'] = data.edge_index.t()[perm[num_val:num_test+num_val], :]
    train_edge['edge'] = data.edge_index.t()[perm[num_test+num_val:], :]
    perm_neg = torch.randperm(num_val+num_test)
    valid_edge['edge_neg'] = neg_edge.t()[perm_neg[:num_val], :]
    test_edge['edge_neg'] = neg_edge.t()[perm_neg[num_val:num_val+num_test], :]
    split_edge['train'] = train_edge
    split_edge['valid'] = valid_edge
    split_edge['test'] = test_edge
...
def get_data(args):
    ...
    #largest_connected_component, in order to keep dataset the same, thus change to disabled
    # use_lcc_flag = True
    # use_lcc_flag = False
    directed = False
    # eval_metric = 'hits'
    ...
    #change graph data from dataset.data to dataset
    if dataset_name in ('ogbn-arxiv', 'reddit', 'flickr'):
        dataset_str = '../dataset/' + dataset_name + '/'
        g = sp.load_npz(dataset_str + 'adj_full.npz')
        if dataset_name == 'ogbn-arxiv':
            g = g + g.T
            g[g > 1] = 1
        #A = g.toarray()
        from torch_geometric.utils import from_scipy_sparse_matrix
        edge_index, _ = from_scipy_sparse_matrix(g)
        #edge_index, _ = dense_to_sparse(torch.tensor(A))
        features = np.load(dataset_str + 'feats.npy')
        dataset = Data(edge_index=edge_index, x=torch.tensor(features, dtype=torch.float))
        root = f'{dataset_str}elph_'
    elif dataset_name.startswith('ogbl'):
        if dataset_name == 'ogbl-collab' or dataset_name == 'ogbl-ppa':
            eval_metric = 'hits'
        ...
        root = f'{ROOT_DIR}/dataset/{dataset_name}/elph_'
        ...
        split_index = str(0)
        splitstr = ROOT_DIR + '/dataset/new_data_splits/' + dataset_name + '_split_0.6_0.2_' + split_index + '.npz'
        g, features, labels, _, _, _, num_features, num_labels = new_load_data(dataset_name, splitstr)
        A = g.toarray()
        edge_index, _ = dense_to_sparse(torch.tensor(A))
        dataset = Data(edge_index=edge_index, x=features.to(torch.float))
        root = f'{ROOT_DIR}/dataset/new_data_splits/elph_'
    ...
    if dataset_name in ('ogbn-arxiv', 'reddit', 'flickr'):
        split_edge = split_edges_gc(dataset, args)
        splits = get_ogb_data(dataset, split_edge, dataset_name, args.num_negs)
        deg = torch.zeros(dataset.num_nodes)
        ob_links = split_edge['train']['edge']
        for i in range(ob_links.shape[0]):
            deg[ob_links[i,0]] += 1
            deg[ob_links[i,1]] += 1
    elif dataset_name.startswith('ogbl'):  # use the built in splits
        split_edge = dataset.get_edge_split()
        dataset = dataset[0]  # the same datatype as Planetoid or others
        deg = torch.zeros(dataset.num_nodes)
        if dataset_name == 'ogbl-citation2':
            ob_links_s = split_edge['train']['source_node']
            ob_links_t = split_edge['train']['target_node']
            for i in range(ob_links_s.shape[0]):
                deg[ob_links_s[i]] += 1
                deg[ob_links_t[i]] += 1
        else:
            ob_links = split_edge['train']['edge']
        ...
        if dataset_name == 'ogbl-collab' and args.year > 0:  # filter out training edges before args.year
            dataset, split_edge = filter_by_year(dataset, split_edge, args.year)
        splits = get_ogb_data(dataset, split_edge, dataset_name, args.num_negs)
        ...
        #Data(x=[2708, 1433], edge_index=[2, 7392], y=[2708], train_mask=[2708], val_mask=[2708], test_mask=[2708], edge_label=[number], edge_label_index=[2, number])
        train_data, val_data, test_data = transform(dataset)#.data)

        row, col = train_data.edge_index
        adj = torch.zeros(train_data.num_nodes, train_data.num_nodes)
        adj[row, col] = 1
        adj[col, row] = 1
    ...
    return dataset, splits, directed, eval_metric, root, deg
