...
class LinkPredictor_deg(torch.nn.Module):
    def __init__(self, args, use_embedding=False):
        super(LinkPredictor_deg, self).__init__()
        self.use_embedding = use_embedding
        self.use_feature = args.use_feature
        self.feature_dropout = args.feature_dropout
        self.label_dropout = args.label_dropout
        self.dim = args.max_hash_hops * (args.max_hash_hops + 2)
        self.label_lin_layer = Linear(self.dim, self.dim)
        self.deg_proj_layer = Linear(1, self.dim)
        ...
        if args.use_feature:
            ...
            self.lin_out_deg = Linear(1, args.hidden_channels)
            self.lin_out_deg2 = Linear(args.hidden_channels, args.hidden_channels)
        ...
        self.lin = Linear(out_channels, 1)
        self.out_stru = Linear(self.dim, 1)
        self.out_feat = Linear(args.hidden_channels, 1)
        self.lin1_deg = Linear(1,args.hidden_channels)
        self.lin2_deg = Linear(args.hidden_channels, 1)
    ...
    def forward(self, sf, sd, node_features, stru_enh, sco_enh, dot_enh, deg, emb=None):
        ...
        if sco_enh == 0:
            if self.use_feature:
                if dot_enh != 0 :
                    fea_dotpro = (node_features[:, 0, :] * node_features[:, 1, :]).sum(dim=-1)
                node_features = self.feature_forward(node_features)
                x = torch.cat([x, node_features.to(torch.float)], 1)
            if emb is not None:
                node_embedding = self.embedding_forward(emb)
            ...
            if dot_enh == 0:
                ...
                x = self.lin(x) + fea_dotpro.view(-1, 1) * self.lin2_deg(F.relu(self.lin1_deg((deg[:, 0] + deg[:, 1]).view(-1, 1))))
        else:
            if self.use_feature:
                node_features = (node_features[:, 0, :] * node_features[:, 1, :]).sum(dim=-1)
            if sco_enh == 0:
                x = self.out_stru(x) + node_features.view(-1,1)
            else:
                x = node_features.view(-1, 1) + self.out_stru(x) * self.lin2_deg(F.relu(self.lin1_deg((deg[:, 0] + deg[:, 1]).view(-1, 1))))
...
class ELPH(torch.nn.Module):
    ...
    def __init__(self, args, num_features, node_embedding=None):
        ...
        self.label_dropout = args.label_dropout
        self.feature_dropout = args.feature_dropout
        self.num_layers = args.max_hash_hops#2
        self.dim = args.max_hash_hops * (args.max_hash_hops + 2)
        ...
        self.predictor = LinkPredictor_deg(args, node_embedding is not None)
    ...
    def _convolution_builder(self, num_features, out_channels, args):
        hidden_channels = 64
