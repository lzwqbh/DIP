...
def train_gae(model, optimizer, train_loader, args, device, deg):
    """
    ...
    model.train()
    total_loss = 0
    #exactly the original dataset, which means there are many useless codes before
    data = train_loader.dataset
    # hydrate edges
    links = data.links
    labels = torch.tensor(data.labels)
    ...
    loader = DataLoader(range(len(links)), args.batch_size, shuffle=True)
    if args.no_adj > 0.5:
        adj_empty = torch.tensor([[], []], dtype=torch.int64).to(device)
    for batch_count, indices in enumerate(tqdm(loader)):
    # for batch_count, indices in enumerate(loader):
        ...
        if args.no_adj > 0.5:
            ...
            if args.dataset_name == 'ogbl-ppa':
                x_cur = data.x[curr_links].to(torch.float32).to(device)
            else:
                x_cur = data.x[curr_links].to(device)
            x_s = model(x_cur[:,0,:], adj_empty)
            x_t = model(x_cur[:, 1, :], adj_empty)
            logits = torch.sigmoid((x_s * x_t).sum(dim=1))

            y = labels[indices].squeeze(0).to(device)
            pos_loss = -torch.log(logits[y == 1] + EPS).mean()
            neg_loss = -torch.log(1 - logits[y == 0] + EPS).mean()
            optimizer.zero_grad()
...
def train_elph(model, optimizer, train_loader, args, device, deg):
    """
    train a GNN that calculates hashes using message passing
    @param model:
    @param optimizer:
