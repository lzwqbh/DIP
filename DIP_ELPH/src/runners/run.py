...
from src.models.gaes import GAE
...
def print_results_list(results_list):
    for idx, res in enumerate(results_list):
        print(f'repetition {idx}: test {100 * res[0]:.2f}, val {100 * res[1]:.2f}, train {100 * res[2]:.2f}')
...
def run(args):
    #for online sharing and recording
    ...
    for rep in range(args.reps):
        set_seed(rep)
        dataset, splits, directed, eval_metric, root, deg = get_data(args)
        train_loader, train_eval_loader, val_loader, test_loader = get_loaders(args, dataset, splits, directed, root)
        ...
        #pretrained or embedding changed while training, not used
        ...
        if eval_metric != 'auc':
            ...
            val_auc = test_auc = val_ap = test_ap = best_epoch = 0
        print(f'running repetition {rep}')
        if rep == 0:
            print_model_params(model)
        for epoch in range(args.epochs):
            t0 = time.time()
            loss = train_func(model, optimizer, train_loader, args, device, deg)
            if (epoch + 1) % args.eval_steps == 0:
                results, tmp_std_scores = test(model, evaluator, train_eval_loader, val_loader, test_loader, args, device, deg,
                ...
                if eval_metric != 'auc':
                    ...
                    if tmp_val_auc > val_auc:
                        ...
                        test_ap = tmp_test_ap
                        best_epoch = epoch
                        std_scores = tmp_std_scores
                        weights = deepcopy(model.state_dict())
                    to_print = f'Epoch: {epoch:02d}, Best epoch: {best_epoch}, Loss: {loss:.4f}, Valid AUC: {100 * val_auc:.2f}%, Test AUC: ' \
                            f' {100 * test_auc:.2f}%, Valid AP: {100 * val_ap:.2f}%, Test AP: {100 * test_ap:.2f}%, epoch time: {time.time() - t0:.1f}'
                    print(to_print)

        if args.no_adj > 0.5:
            torch.save(weights, f'saved_para/para_{args.dataset_name}_{rep}.pt')
        if args.reps > 1:
            if eval_metric != 'auc':
                ...
                std_scores_list.append([std_scores])
                for idx, res in enumerate(results_list):
    ...
    if args.reps > 1:
        if eval_metric != 'auc':
            ...
            print('test_acc_mean: ', test_acc_mean, 'val_acc_mean: ', val_acc_mean, 'train_acc_mean: ', train_acc_mean)
            print('test_acc_std: ', test_acc_std, 'val_acc_std: ', val_acc_std)
        else:
            ...
            std_scores_mean = np.mean(std_scores_list,axis=0)
            print('mean std scores:', std_scores_mean, 'scores:', std_scores_list)
...
def select_model(args, dataset, emb, device):
    if args.model == 'SEALDGCNN':
        ...
        if args.dataset_name == 'ogbl-ppa' or args.dataset_name == 'ogbl-citation2':
            model = GAE(dataset.num_features, 8, 8).to(device)
        elif args.dataset_name == 'reddit':
            model = GAE(dataset.num_features, 16, 16).to(device)
        else:
            model = GAE(dataset.num_features, 64, 32).to(device)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset-name', type=str, default='cora',
                        choices=['cora', 'citeseer', 'pubmed', 'ogbl-ppa', 'ogbl-collab', 'ogbl-ddi',
                                 'ogbl-citation2', 'cornell', 'wisconsin', 'texas', 'ogbn-arxiv', 'flickr', 'reddit'])
    parser.add_argument('--val_pct', type=float, default=0.05,
                        help='the percentage of supervision edges to be used for validation. These edges will not appear'
                             ' in the training set and will only be used as message passing edges in the test set')#default=0.1
    parser.add_argument('--test_pct', type=float, default=0.1,
                        help='the percentage of supervision edges to be used for test. These edges will not appear'
                             ' in the training or validation sets for either supervision or message passing')#default=0.2
    ...
    # GNN settings
    parser.add_argument('--model', type=str, default='ELPH')#'BUDDY')
    parser.add_argument('--hidden_channels', type=int, default=32)#1024)
    ...
    parser.add_argument('--stru_enh', type=int, default=0,
                        help='whether enhancing structure')
    parser.add_argument('--sco_enh', type=int, default=0,
                        help='whether enhancing score')
    parser.add_argument('--dot_enh', type=int, default=0,
                        help='whether enhancing with dot product')

    parser.add_argument('--no_adj', type=int, default=0,
                        help='whether do not use adj')
    ...
    parser.add_argument('--data_appendix', default=None, type=str)
