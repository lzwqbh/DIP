...
def get_train_val_test_datasets(dataset, train_data, val_data, test_data, args, root):
    sample = 'all' if not args.sample_size else args.sample_size
    path = f'{root}_seal_{sample}_hops_{args.num_hops}_maxdist_{args.max_dist}_mnph_{args.max_nodes_per_hop}{args.data_appendix}'
