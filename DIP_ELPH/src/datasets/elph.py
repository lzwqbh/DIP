...
class HashDataset(Dataset):
    ...
    def __init__(
        ...
        if args.model != 'ELPH' or args.model != 'gae':  # elph stores the hashes directly in the model class for message passing
        ...
        if args.model == 'ELPH' or args.model == 'gae':  # features propagated in the model instead of preprocessed
            self.x = data.x
        else:
    ...
    def _preprocess_subgraph_features(self, device, num_nodes, num_negs=1):
        ...
        if self.args.floor_sf and self.subgraph_features is not None:
            ...
            #print(f'setting {torch.sum(self.subgraph_features[self.subgraph_features < 0]).item()} negative values to zero')
...
def get_hashed_train_val_test_datasets(dataset, train_data, val_data, test_data, args, root, directed=False):
