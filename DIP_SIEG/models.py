...
class DGCNNGraphormer_noNeigFeat(torch.nn.Module):
    def __init__(self, args, hidden_channels, num_layers, max_z, k=0.6, train_dataset=None,
        ...
        self.use_feature = use_feature
        self.use_feature_GT = use_feature_GT#default True
        self.node_embedding = node_embedding
        self.readout_type = readout_type#default 0

        if k <= 1:  # Transform percentile to number.default 0.6
            if train_dataset is None:
                k = 30
            else:
                num_nodes = []
        ...
        if args.use_ignn:#default false
            self.convs.append(NGNN(initial_channels, hidden_channels, hidden_channels))
            for i in range(0, num_layers-1):
                self.convs.append(NGNN(hidden_channels, hidden_channels, hidden_channels))
            self.convs.append(NGNN(hidden_channels, hidden_channels, 1))
        ...
        dense_dim = int((self.k - 2) / 2 + 1)
        dense_dim = (dense_dim - conv1d_kws[1] + 1) * conv1d_channels[1]
        mlp_hidden_channels = args.mlp_hidden_channels#default 128
        self.lin_align = Linear(dense_dim, mlp_hidden_channels)
        if self.readout_type == 0:#default 0
            self.lin1 = Linear(2*mlp_hidden_channels, mlp_hidden_channels)
        elif self.readout_type == 1:
            h = torch.cat((h_src, h_dst, h, h_src_graphormer, h_dst_graphormer), dim=-1)
            self.lin1 = Linear(2*total_latent_dim + 3*mlp_hidden_channels, mlp_hidden_channels)
        ...
        deg_hid = 128
        self.lin1_deg = Linear(1, deg_hid)#elph 32,wp 72
        self.lin2_deg = Linear(deg_hid, 1)
        # self.lin02_deg = Linear(hidden_channels, deg_hid)
        self.conv1_deg = GNN(feat_channels, hidden_channels)
        self.conv2_deg = GNN(hidden_channels, deg_hid)

    def forward(self, data, dot_enh):
        x = data.x
        z = data.z#guess drnl
        edge_index = data.edge_index
        batch = data.batch#size=[number nodes], indicates the batch of training subgraph vertex set
        edge_weight = data.edge_weight
        # node_id = data.node_id
        node_id = None
        ...
        if z_emb.ndim == 3:  # in case z has multiple integer labels
            z_emb = z_emb.sum(dim=1)
        h = z_emb#size=[number nodes,128]
        ffn_feat = x.to(torch.float)

        if self.node_embedding is not None and node_id is not None:#False
            n_emb = self.node_embedding(node_id)
            h = torch.cat([h, n_emb], 1)
        if self.use_rpe:#default false
            x_rpe = data.x_rpe
            x_rpe_emb = self.trainable_embedding(x_rpe).view(x_rpe.shape[0], -1)
            h = torch.cat([h, x_rpe_emb], 1)
        hs = [h]
        for conv in self.convs:
            hs += [torch.tanh(conv(hs[-1], edge_index, edge_weight))]
        h = torch.cat(hs[1:], dim=-1)  # h: [num_nodes, 3*input_dim+1] sum all the past 4 conv results

        # linear -> concat -> linear
        h_feature = F.relu(self.lin01(ffn_feat))  # FFN单独编码feature
        h = torch.cat((h, h_feature), dim=1)
        h = F.relu(self.lin02(h))# h: [num_nodes, 3*input_dim+1(385)]

        _, center_indices = np.unique(batch.cpu().numpy(), return_index=True)#return the first position that batch start

        if dot_enh:
            # emb_src = h[center_indices]
            # emb_dst = h[center_indices + 1]

            # h_deg = self.lin02_deg(h_feature)

            h_deg = F.relu(self.conv1_deg(ffn_feat, edge_index, edge_weight))
            h_deg = self.conv2_deg(h_deg, edge_index, edge_weight)
            emb_src = h_deg[center_indices]
            emb_dst = h_deg[center_indices + 1]

        if self.readout_type:#default 0 false
            h_src = h[center_indices]
            h_dst = h[center_indices + 1]

        # Global pooling.
        h = global_sort_pool(h, batch, self.k)  # h: [num_graphs, k * hidden] [64, 3850]
        h = h.unsqueeze(1)  # [num_graphs, 1, k * hidden] [64, 1, 3850]
        h = F.relu(self.conv1(h))#[64,16,k(10)]
        h = self.maxpool1d(h)
        # [64, 16, 5]
        h = F.relu(self.conv2(h))
        # [64, 32, 1]
        ...
        # Graphormer embedding
        if self.use_feature_GT:#default True
            pair_data = abstract_pair_data(data)
        else:
            z_emb_src = z_emb[center_indices]
            z_emb_dst = z_emb[center_indices + 1]
        ...
        h = self.lin2(h)
        if dot_enh:
            h = h + (emb_src * emb_dst).sum(dim=-1).view(-1,1) * self.lin2_deg(F.relu(self.lin1_deg((pair_data.undir_degree[:,0] + pair_data.undir_degree[:,1]).view(-1, 1).to(torch.float))))
        return h
